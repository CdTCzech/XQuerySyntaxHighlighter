...
namespace XQuerySyntaxHighlighter
{
	...
	{
		public BraceMatchingTag(string type) : base(type) { }
	}

	[Export(typeof(EditorFormatDefinition))]
	[Name("XQSH/BraceMatching")]
	[UserVisible(true)]
	internal class BraceMatchingFormat : MarkerFormatDefinition
	{
	...
	internal class BraceMatchingTagger : ITagger<TextMarkerTag>
	{
		...
		public IEnumerable<ITagSpan<TextMarkerTag>> GetTags(NormalizedSnapshotSpanCollection spans)
		{
			...
			if (m_braceList.ContainsKey(currentText))   //the key is the open brace
			{
				...
				{
					yield return new TagSpan<TextMarkerTag>(new SnapshotSpan(currentChar, 1), new TextMarkerTag("XQSH/BraceMatching"));
					yield return new TagSpan<TextMarkerTag>(pairSpan, new TextMarkerTag("XQSH/BraceMatching"));
				}
			}
			else if (m_braceList.ContainsValue(lastText))    //the value is the close brace, which is the *previous* character 
			{
				...
				{
					yield return new TagSpan<TextMarkerTag>(new SnapshotSpan(lastChar, 1), new TextMarkerTag("XQSH/BraceMatching"));
					yield return new TagSpan<TextMarkerTag>(pairSpan, new TextMarkerTag("XQSH/BraceMatching"));
				}
			}
		}
